...
export class CohortResult {
  ...
  public saveCohort() {
    cy.get('.button-save > .mat-mdc-tooltip-trigger').contains('Save').click()
    cy.get('num-save-dataquery-modal').should('not.exist')
  }
}

export const cohortResult = new CohortResult()
defineStep('I see the spinner is visible', () => {
  cohortResult.spinnerIsVisible()
})
defineStep('I see the spinner is not visible', () => {
  cohortResult.spinnerIsNotVisible()
})
defineStep('I see the result after {int} seconds', (seconds: number) => {
  cohortResult.resultIsVisible(seconds)
})
defineStep('I open the details', () => {
  cohortResult.openDetailsDialog()
})
defineStep('I close the details dialog', () => {
  cohortResult.closeDetailsDialog()
})
defineStep('I see a dialog', () => {
  cohortResult.openSaveDialog()
})
...
defineStep('I save the cohort', () => {
  cohortResult.saveCohort()
})
