...
export class CohortEdit {
  ...
  public selectFromPanelByName(option: string, panelName: string) {
    cy.get('num-quantity-comparision-select').click().then(() => {
...
defineStep('I select {string} from the panel with the name {string}', (option: string, panelName: string) => cohortEdit.selectFromPanelByName(option, panelName))
defineStep('I click on Auswählen button', () => cohortEdit.clickAuswaehlenButton())
defineStep('I should see {string} modal closing', (modalTitle: string) => cohortEdit.shouldSeeModalClosing(modalTitle))
