...
export class RefrenceCriterionMenuFunctionsService {
  ...
  deleteCriterion(uid: string) {
    ...
    this.criterionService.getCriterionByUID(parentID).getAttributeFilters().forEach((attributeFilter) => {
      console.log(attributeFilter)
      if (attributeFilter.isReferenceSet()) {
        const updatedReferences = attributeFilter.getReference().getSelectedReferences().filter((reference) => reference.getId() !== uid);
  ...
  applyReferenceCriterionFilter(uid: string) {
    this.editCriterionService.openCriterionModal(this.referenceCriterionProvider.getReferenceCriterionByUID(uid));
  }
}
